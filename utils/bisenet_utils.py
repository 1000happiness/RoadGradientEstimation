...
class ToTensor(object):
    ...
    '''
    def __init__(self, device, mean=(0, 0, 0), std=(1., 1., 1.)):
        self.device = device
        self.mean = torch.as_tensor(mean, dtype=torch.float32, device=device)[:, None, None]
        self.std = torch.as_tensor(std, dtype=torch.float32, device=device)[:, None, None]

    def __call__(self, im):
        im = im.transpose(2, 0, 1).astype(np.float32)
        im = torch.from_numpy(im).div_(255).to(self.device)
        im = im.sub_(self.mean).div_(self.std).clone()
        return im
