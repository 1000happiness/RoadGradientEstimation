...
parse.add_argument('--pitch', type=float, default=0)
...
def show(args):
    ...
    for row_idx, row in gt_content.iterrows():
        if args.max_frame and row['frame_idx'] > args.max_frame:
            break

        ret, front_row_index, front_distance = find_x_meters_front_row_idx(30, gt_content, front_row_index, front_distance)
        if not ret:
            break
        ...
        if str(int(row['frame_idx'])) in log_content:
            ...
            last_log_angle_array = params[0] * raw_log_depth_array * raw_log_depth_array + params[1] * raw_log_depth_array + params[2] + args.pitch

        depth_delta = gt_content[(gt_content['frame_idx'] > last_log_frame_idx) & (gt_content['frame_idx'] <= row['frame_idx'])]['distance'].sum()

        log_depth_list = last_log_depth_array - depth_delta
        log_angle_list = last_log_angle_array

        for i in range(0, 30):
            if len(last_log_depth_array) > 0 and i > last_log_depth_array.min() and i < last_log_depth_array.max():
                pred_angle = params[0] * i * i + params[1] * i + params[2] + args.pitch
        ...
        if args.show_table:
            ...
            plt.plot(log_depth_list, log_angle_list, c='b')
            plt.plot(raw_log_depth_array, raw_log_angle_array + args.pitch, c='b')
            plt.pause(0.033)

        front_distance -= row['distance']

        print(row_idx, num_less_than_2_5, num_less_than_5, num_less_than_7_5, num_less_than_10, num_more_than_10, sep="| ")
